...
function generatetransaction(idx, backaddr, amountback, utxo, message, ourwif) {
    ...
    let generate_cmnd = `./my_addr -idx ${idx.replace(/\s/g,'')} -backaddr ${backaddr.replace(/\s/g,'')} -backsats ${amountback.replace(/\s/g,'')} -utxo ${utxo.replace(/\s/g,'')} -msg '${message}' -wif ${ourwif.replace(/\s/g,'')} -onlytx`
